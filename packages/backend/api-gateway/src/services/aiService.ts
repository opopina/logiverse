...
export interface AIResponse {
  text: string;
  emotion: 'happy' | 'encouraging' | 'thinking' | 'celebrating' | 'challenging' | 'surprised' | 'friendly';
...
const LOGGIE_PERSONALITY: LoggiePersonality = {
  ...
  speakingStyle: "Amigable, claro y educativo. Usa emojis ocasionalmente. Explica conceptos complejos de forma simple."
};
...
Responde como Loggie ayudaría en esta situación específica.`;
  ...
  private determineEmotionAndAccessory(context: LoggieContext, responseType: string): { emotion: AIResponse['emotion'], accessory: AIResponse['accessory'] } {
    ...
    if (context.levelType === 'logic-puzzle' || context.levelType === 'syllogism') {
  ...
  async generateResponse(
    ...
    responseType: 'level-start' | 'hint' | 'correct' | 'incorrect' | 'encouragement' | 'explanation',
    customPrompt?: string
  ): Promise<AIResponse> {
    try {
      let userPrompt = '';
      ...
      switch (responseType) {
        ...
        case 'explanation':
          userPrompt = customPrompt || `Explica el concepto detrás del nivel "${context.levelTitle}" de tipo ${context.levelType}.`;
          break;
      }
      ...
      const responseText = completion.choices[0]?.message?.content || "¡Hola! Soy Loggie y estoy aquí para ayudarte con la lógica! 🦊";
      const { emotion, accessory } = this.determineEmotionAndAccessory(context, responseType);

      return {
        text: responseText,
        emotion,
        accessory,
        tone: responseType === 'correct' ? 'excited' : 
              responseType === 'incorrect' ? 'encouraging' : 
              responseType === 'hint' ? 'helpful' : 
              responseType === 'encouragement' ? 'motivational' : 'analytical'
      };
      ...
      const fallbackResponses = {
        'level-start': "¡Hola! Soy Loggie y estoy emocionado de resolver este desafío contigo. ¡Vamos a usar la lógica! 🦊",
        'hint': "Aquí tienes una pista: piensa paso a paso y analiza cada parte del problema. ¡Tú puedes! 💡",
        'correct': "¡Increíble! Lo lograste. Tu razonamiento lógico está mejorando cada vez más. ¡Sigue así! 🎉",
        'incorrect': "No es la respuesta correcta, pero no te rindas. Cada error es una oportunidad de aprender. ¡Inténtalo de nuevo! 🦊",
        'encouragement': "La lógica requiere práctica y paciencia. ¡Estás en el camino correcto! Sigue intentando. 💪",
        'explanation': "Te ayudo a entender este concepto paso a paso. La lógica es como un rompecabezas que resolvemos juntos. 🧩"
      };

      return {
        text: fallbackResponses[responseType] || "¡Estoy aquí para ayudarte con la lógica! 🦊",
        emotion: 'friendly',
        accessory: 'scarf',
        tone: 'helpful'
      };
  ...
  async generateCustomHint(context: LoggieContext, specificQuestion: string): Promise<AIResponse> {
  ...
  async generatePersonalizedFeedback(context: LoggieContext, userHistory: any): Promise<AIResponse> {
